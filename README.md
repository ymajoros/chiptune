...
`web/` is a self-contained, **no-samples** web MIDI player built from the same
engine: a static page that parses `.mid` files and synthesises them live with
the Web Audio API — a mixer, per-channel GM instruments, an effects rack
(reverb / delay / chorus), a per-voice instrument editor, and a playable
keyboard (computer keys or a real Web MIDI device). No sound-fonts, no samples;
every sound is generated by the hand-written synth.

```bash
node build.mjs            # dev build      -> web/app.js  (bundles songData.ts)
node build.mjs --serve    # dev + watch + http://localhost:8080
node build.mjs --release  # self-contained release -> dist/
```

The **release** build writes a static `dist/` folder (`index.html` + minified
`app.js` + `LICENSE` + `README.txt`) that you serve with any static web server.
It bundles an original, public-domain demo song
([`web/demoSong.ts`](web/demoSong.ts)) **instead of** `songData.ts`, so the
distributable carries none of the copyrighted note data. To share it, zip the
`dist/` folder. See [`dist/README.txt`](dist/README.txt) (created by the build)
for end-user instructions.

## Licence / contents

Code is **MIT** licensed — see [`LICENSE`](LICENSE). (Prefer a different licence
for your own fork? Swap that file.)
...
personal experiment. Don't redistribute them — and note the web release above
